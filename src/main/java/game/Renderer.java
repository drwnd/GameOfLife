...
public final class Renderer extends Renderable {
    ...
    public void renderSelf(Vector2f position, Vector2f size) {
        ...
        if (ToggleSetting.SIMULATION_RUNNING.value()) {
            ...
            glBindImageTexture(0, texture0, 0, false, 0, GL_WRITE_ONLY, GL_RGBA32UI);
            glBindImageTexture(1, texture1, 0, false, 0, GL_READ_ONLY, GL_RGBA32UI);
            glDispatchCompute(1 << SIZE_BITS - 7, 1 << SIZE_BITS - 6, 1);
        ...
        glBindImageTexture(0, texture1, 0, false, 0, GL_READ_WRITE, GL_RGBA32UI);
    ...
    private static int genTexture(int[] data) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32UI, 1 << SIZE_BITS - 7, 1 << SIZE_BITS, 0, GL_RGBA_INTEGER, GL_UNSIGNED_INT, data);
